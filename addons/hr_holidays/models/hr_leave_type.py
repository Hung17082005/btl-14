...
class HolidaysType(models.Model):
    ...
    def _get_employees_days_per_allocation(self, employee_ids, date=None):
        ...
        future_allocations_date_to = fields.datetime.combine(
            date, time.max
        ...
        for employee_id, allocation_intervals_by_status in allocation_employees.items():
            ...
            for holiday_status_id, intervals in allocation_intervals_by_status.items():
                ...
                for (
                    interval_from,
                    interval_to,
                    interval_allocations,
                ) in future_allocation_intervals._items:
                    ...
                    employee_quantity_available = (
                        employee._get_work_days_data_batch(
                            interval_from,
                            interval_to,
    ...
    def get_employees_days(self, employee_ids, date=None):
        ...
        for employee_id in allocations_days_consumed:
            for holiday_status_id in allocations_days_consumed[employee_id]:
                if allocations_days_consumed[employee_id][holiday_status_id].get(
                    "error"
                ):
                ...
                for allocation in allocations_days_consumed[employee_id][
                    ...
                    if (
                        ...
                        and (allocation.date_to < date or allocation.date_from > date)
                    ):
                        continue
                    for leave_key in leave_keys:
                        result[employee_id][
    ...
    def _get_days_request(self):
        ...
        return (
            self.name,
            {
                ...
                .rstrip("0")
                .rstrip("."),
                "usable_remaining_leaves": ("%.2f" % self.virtual_remaining_leaves)
                .rstrip("0")
                .rstrip("."),
                "virtual_remaining_leaves": (
                    "%.2f" % (self.max_leaves - self.virtual_leaves_taken)
                )
                .rstrip("0")
                .rstrip("."),
