...
class HolidaysRequest(models.Model):
    ...
    def default_get(self, fields_list):
        ...
        if defaults.get("request_date_from") and defaults.get("request_date_to"):
            default_attendance_from, default_attendance_to = self._get_attendances(
                employee, defaults["request_date_from"], defaults["request_date_to"]
            )
            defaults["date_from"] = self._get_start_or_end_from_attendance(
                default_attendance_from.hour_from,
                defaults["request_date_from"],
                employee,
            )
            defaults["date_to"] = self._get_start_or_end_from_attendance(
                default_attendance_to.hour_to, defaults["request_date_to"], employee
            )
        ...
        if "state" in fields_list and not defaults.get("state"):
            ...
            defaults["state"] = (
                "confirm"
                if lt and lt.leave_validation_type != "no_validation"
                else "draft"
            )
        ...
        if (
            defaults["date_from"].time() == default_start_time
            and defaults["date_to"].time() == default_end_time
        ):
    ...
    def _check_holidays(self):
        mapped_days = self.holiday_status_id.get_employees_days(
            (self.employee_id | self.sudo().employee_ids).ids
        )
        for holiday in self:
            if (
