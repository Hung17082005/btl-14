        ...
        for logs, (periodicity, run_date), (exp_periodicity, exp_run_date) in zip(
            [
                ...
                [
                    (
                        self.user_employee,
                        self.reference_datetime - relativedelta(days=4),
                    )
                ],  # old logs -> tone down
                [],  # no logs -> tone down
                # weekly
            ...
            with self.subTest(logs=logs, periodicity=periodicity, run_date=run_date):
                ...
                with self.mock_datetime_and_now(run_date), self.mock_mail_gateway():
                    digest.action_send()
...
class TestUnsubscribe(mail_test.MailCommon, HttpCaseWithUserDemo):
    ...
    def _url_unsubscribe(self, token=None, user_id=None, one_click=None, method="GET"):
        ...
        url = url_join(
            self.base_url,
            f"digest/{self.test_digest.id}/{unsubscribe_route}?{url_encode(url_params)}",
        )
        if method == "GET":
