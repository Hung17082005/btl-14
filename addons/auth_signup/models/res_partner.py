...
class ResPartner(models.Model):
    ...
    def init(self):
        ...
        if not sql.column_exists(self.env.cr, self._table, "signup_token"):
            self.env.cr.execute(
                "ALTER TABLE res_partner ADD COLUMN signup_token varchar"
            )
    ...
    def _compute_token(self):
        for partner in self.filtered("id"):
            self.env.cr.execute(
                "SELECT signup_token FROM res_partner WHERE id=%s",
                (partner._origin.id,),
            )
    ...
    def _inverse_token(self):
        for partner in self.filtered("id"):
            self.env.cr.execute(
                "UPDATE res_partner SET signup_token = %s WHERE id=%s",
                (partner.signup_token or None, partner.id),
            )
    ...
    def _signup_retrieve_partner(
        ...
        """
        self.env.cr.execute(
            "SELECT id FROM res_partner WHERE signup_token = %s AND active", (token,)
        )
