...
class Job(models.Model):
    ...
    no_of_hired_employee = fields.Integer(
        compute="_compute_no_of_hired_employee", store=True
    )

    @api.depends("application_ids.date_closed")
    def _compute_no_of_hired_employee(self):
        for job in self:
            job.no_of_hired_employee = len(
                job.application_ids.filtered(lambda applicant: applicant.date_closed)
            )
            ...
            defaults.update(
                {
                    ...
                    "company_id": (
                        self.department_id.company_id.id
