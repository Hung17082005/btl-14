    ...
    def test_sftp_connection(self, context=None):
        ...
        for rec in self:
            ...
            try:
                ...
                s.connect(
                    ip_host, port_host, username_login, password_login, timeout=10
                )
                sftp = s.open_sftp()
                sftp.close()
                message_title = _(
                ...
                _logger.critical(
                    "There was a problem connecting to the remote ftp: %s", str(e)
                )
                error += str(e)
                has_failed = True
            ...
            if rec.sftp_write is True:
                try:
                        ...
                        if rec.name in f:
                            ...
                            if os.path.isfile(fullpath):
                                ...
                                # This means the file does not exist (remote) yet!
                                    ...
                                    try:
                                        sftp.put(
                                            fullpath, os.path.join(path_to_write_to, f)
                                        )
                                        _logger.info(
                                        ...
                                        _logger.critical(
                                            "We couldn't write the file to the remote server. Error: %s",
                                            str(err),
                                        )
                        ...
                        if rec.name in file:
                            ...
                            if delta.days >= rec.days_to_keep_sftp:
                                ...
                                if ".dump" in file or ".zip" in file:
                                    _logger.info(
                                        "Delete too old file from SFTP servers: %s",
                                        file,
                                    )
                                    sftp.unlink(file)
                    ...
                    try:
                        ...
                        pass
                    _logger.error(
                        "Exception! We couldn't back up to the FTP server. Here is what we got back "
                        "instead: %s",
                        str(e),
                    )
                    ...
                    if rec.send_mail_sftp_fail:
                        try:
                            ...
                            response_mail = (
                                "auto_backup@%s" % catch_all_domain
                                if catch_all_domain
                    ...
                    if rec.name in fullpath:
                        ...
                        if delta.days >= rec.days_to_keep:
                            ...
                            if os.path.isfile(fullpath) and (
                                ...
                                _logger.info(
                                    "Delete local out-of-date file: %s", fullpath
                                )
                                os.remove(fullpath)
