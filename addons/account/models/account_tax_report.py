...
class AccountTaxReportLine(models.Model):
    ...
    def create(self, vals):
        ...
        if tag_name and vals.get("report_id"):
            ...
            if len(existing_tags) < 2:
                ...
                vals["tag_ids"] = self._get_tags_create_vals(
    ...
    def _remove_tags_used_only_by_self(self):
        ...
        for tag in tags_to_unlink:
            ...
            if aml_using_tags:
                rep_lines_with_archived_tags = (
                    self.env["account.tax.repartition.line"]
                    .sudo()
                    .search([("tag_ids", "in", tag.id)])
                )
    ...
    def _validate_tags(self):
        for record in self.filtered(lambda x: x.tag_ids):
            ...
            if len(neg_tags) > 1 or len(pos_tags) > 1:
                raise ValidationError(
                    _(
                        "If tags are defined for a tax report line, only two are allowed on it: a positive and/or a negative one."
                    )
                )

            if (neg_tags and neg_tags.name != "-" + record.tag_name) or (
                pos_tags and pos_tags.name != "+" + record.tag_name
            ):
                raise ValidationError(
                    _(
