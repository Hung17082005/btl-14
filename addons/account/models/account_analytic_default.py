...
class AccountAnalyticDefault(models.Model):
    ...
    def _check_account_or_tags(self):
        if any(
            not default.analytic_id
            and not any(
                tag.analytic_distribution_ids for tag in default.analytic_tag_ids
            )
            for default in self
        ):
            raise ValidationError(
                _(
                    "An analytic default requires an analytic account or an analytic tag used for analytic distribution."
                )
            )
