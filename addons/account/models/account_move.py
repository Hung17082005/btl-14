...
from odoo.tools.misc import format_date, formatLang, get_lang, groupby
...
class AccountMove(models.Model):
    ...
    display_qr_code = fields.Boolean(
        string="Display QR-code", compute="_compute_display_qr_code"
    )
    ...
    def _onchange_invoice_date(self):
        if self.invoice_date:
            ...
            if (
                self._context.get("force_onchange_currency")
                or accounting_date != self.date
            ):
    ...
    def _onchange_journal(self):
        ...
        if (
            self.state == "draft"
            and self._get_last_sequence(lock=False)
            and self.name
            and self.name != "/"
        ):
            self.name = "/"
        ...
        def _get_payment_terms_account(self, payment_terms_lines):
            ...
            if payment_terms_lines:
                ...
                accounts = {
                    ...
                    for acc, account_type in [
                        (self.partner_id.property_account_payable_id, "payable"),
                        (self.partner_id.property_account_receivable_id, "receivable"),
                    ]
                }
                account_map = (
                    self.fiscal_position_id or self.env["account.fiscal.position"]
    ...
    def _recompute_dynamic_lines(
        ...
        for invoice in self:
            ...
            for line in invoice.line_ids:
                ...
                if line.tax_repartition_line_id:
                    ...
                    if invoice.is_invoice(include_receipts=True):
                        ...
                        is_refund = (tax_type == "sale" and line.debit) or (
                            tax_type == "purchase" and line.credit
                        )
                    taxes = line.tax_ids._origin.flatten_taxes_hierarchy().filtered(
                        lambda tax: (
                            tax.amount_type == "fixed"
                            and not invoice.company_id.currency_id.is_zero(tax.amount)
                            or not float_is_zero(tax.amount, precision_digits=4)
                        )
                    )
                    if is_refund:
                        tax_rep_lines = (
                            taxes.refund_repartition_line_ids._origin.filtered(
                                lambda x: x.repartition_type == "tax"
                            )
                        )
                    else:
                        tax_rep_lines = (
                            taxes.invoice_repartition_line_ids._origin.filtered(
                                lambda x: x.repartition_type == "tax"
                            )
                        )
            ...
            if has_taxes or current_tax_rep_lines:
                ...
                elif delta_tax_rep_lines and not self._context.get(
                    "move_reverse_cancel"
                ):
                    invoice._recompute_tax_lines(
                        tax_rep_lines_to_recompute=delta_tax_rep_lines
                    )
    ...
    def _compute_payment_reference(self):
        for move in self.filtered(
            lambda m: (m.state == "posted" and m._auto_compute_invoice_reference())
        ):
        ...
        if in_invoices or out_invoices or entries:
            for reverse_move in self.env["account.move"].search(
                [
                    ...
                    "&",
                    ("reversed_entry_id", "in", entries.ids),
                    ("move_type", "=", "entry"),
                ]
            ):
    ...
    def _get_move_display_name(self, show_ref=False):
        ...
        return name + (
            show_ref
            and self.ref
            and " (%s%s)" % (self.ref[:50], "..." if len(self.ref) > 50 else "")
            or ""
        )
    ...
    def _reverse_move_vals(self, default_values, cancel=True):
        ...
        for line_command in move_vals.get("line_ids", []):
            ...
            if (
                not is_refund
                or self.tax_cash_basis_origin_move_id
                or self._context.get("reverse_move_type")
            ...
            if line_vals.get("tax_repartition_line_id"):
                ...
                if (
                    invoice_repartition_line.invoice_tax_id.tax_exigibility
                    == "on_payment"
                ):
                    continue
    ...
    def _reverse_moves(self, default_values_list=None, cancel=False):
        ...
        for move, reverse_move in zip(
            ...
            reverse_moves.with_context(
                check_move_validity=False, move_reverse_cancel=cancel
            ),
        ):
    ...
    def message_new(self, msg_dict, custom_values=None):
        ...
        def is_internal_partner(partner):
            ...
            return partner == company.partner_id or (
                partner.user_ids
                and all(user.has_group("base.group_user") for user in partner.user_ids)
            )
        ...
        followers = [
            ...
            for partner in self._mail_find_partner_from_emails(
                cc_mail_addresses, extra_domain=extra_domain
            )
            if partner
        ]
        ...
        senders = partners = [
            partner
            for partner in self._mail_find_partner_from_emails(
                from_mail_addresses, extra_domain=extra_domain
            )
            if partner
        ]
        ...
        if partners:
            ...
            if is_internal_partner(partners[0]):
                ...
                partners = [
                    partner
                    for partner in self._mail_find_partner_from_emails(
                        body_mail_addresses, extra_domain=extra_domain
                    )
        ...
        for move in to_post:
            ...
            if not move.invoice_date:
                if move.is_sale_document(include_receipts=True):
                    ...
                    move.with_context(
                        check_move_validity=False, force_onchange_currency=True
    ...
    def action_switch_invoice_into_refund_credit_note(self):
        for move in self:
            if move.posted_before:
                raise ValidationError(
                    _("You cannot switch the type of a posted document.")
                )
            if move.move_type == "entry":
                raise ValidationError(
                    _("This action isn't available for this document.")
                )
            reversed_move = move.with_context(
                reverse_move_type=True
            )._reverse_move_vals({}, False)
    ...
    def _notify_get_groups(self, msg_vals=None):
        ...
        if self.move_type != "entry":
            ...
            recipient_group = (
                ...
                lambda pdata: pdata["id"] in msg_vals.get("partner_ids", [])
...
class AccountMoveLine(models.Model):
    ...
    def _get_fields_onchange_subtotal(
        ...
        return self._get_fields_onchange_subtotal_model(
            price_subtotal=(
                self.price_subtotal if price_subtotal is None else price_subtotal
            ),
            move_type=self.move_id.move_type if move_type is None else move_type,
            currency=self.currency_id if currency is None else currency,
    ...
    def _get_fields_onchange_balance(
        ...
        return self._get_fields_onchange_balance_model(
            ...
            discount=self.discount if discount is None else discount,
            amount_currency=(
                self.amount_currency if amount_currency is None else amount_currency
            ),
            move_type=self.move_id.move_type if move_type is None else move_type,
            currency=(
                (self.currency_id or self.move_id.currency_id)
                if currency is None
                else currency
            ),
            taxes=self.tax_ids if taxes is None else taxes,
        ...
        for vals in vals_list:
            ...
            if currency_id == move.company_id.currency_id.id:
                ...
                if (
                    ...
                    and vals.get("debit") is False
                    and vals.get("credit") is False
                ):
                    vals["balance"] = move.currency_id._convert(
    ...
    def _prepare_reconciliation_partials(self):
        ...
        void_lines = iter(
            self.filtered(
                lambda line: not line.balance
                and not line.amount_currency
                and not line.reconciled
            )
        )
        debit_line = None
    ...
    def reconcile(self):
        ...
        if (
            is_cash_basis_needed
            and not self._context.get("move_reverse_cancel")
    ...
    def _prepare_analytic_distribution_line(self, distribution):
        ...
        return {
            ...
            "company_id": distribution.account_id.company_id.id
            or self.company_id.id
            or self.env.company.id,
            "category": (
                "invoice"
                if self.move_id.is_sale_document()
                else "vendor_bill" if self.move_id.is_purchase_document() else "other"
            ),
        }
