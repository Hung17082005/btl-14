...
def update_taxes_from_templates(cr, chart_template_xmlid):
    ...
    def _create_taxes_from_template(
        company, template2tax_mapping, template2tax_to_update=None
    ):
        ...
        templates_to_create = env["account.tax.template"].with_context(
            active_test=False
        )
        ...
        new_template2tax_company = templates_to_create._generate_tax(
            company,
            accounts_exist=True,
            existing_template_to_tax=template2tax_to_update,
        )["tax_template_to_tax"]
        return [
            ...
            for position_template in fp_templates:
                ...
                for position_tax in position_template.tax_ids:
                    position_tax_template_exist = fp.tax_ids.filtered(
                        lambda tax_fp: tax_fp.tax_src_id.id
                        == template2tax.get(company_id).get(position_tax.tax_src_id.id)
                        and tax_fp.tax_dest_id.id
                        == (
                    ...
                    if not position_tax_template_exist and (
                        ...
                        fp_tax_template_vals.append(
                            (
                                position_tax,
                                {
                                    "tax_src_id": template2tax.get(company_id).get(
    ...
    for company in companies:
        ...
        new_template2tax += _create_taxes_from_template(
            company, templates_to_tax_create, templates_to_tax_update
        )
...
class AccountTaxTemplate(models.Model):
    ...
    def _generate_tax(
        ...
        tax_template_to_tax = {
            template: tax for (template, tax) in existing_template_to_tax
        }
        ...
        while templates_todo:
            ...
            for template in templates:
                if all(
                    ...
                    if accounts_exist:
                        vals = template._get_tax_vals_complete(
            ...
            for tax, (template, vals) in zip(taxes, tax_template_vals):
                ...
                for existing_template, existing_tax in existing_template_to_tax:
                    if (
                        template in existing_template.children_tax_ids
                        and tax not in existing_tax.children_tax_ids
                    ):
