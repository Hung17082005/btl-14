...
class AccountJournal(models.Model):
    ...
    def _get_journals_payment_method_information(self):
        ...
        manage_acquirers = (
            "payment_acquirer_id" in self.env["account.payment.method.line"]._fields
        )
        ...
        if manage_acquirers:
            ...
            for acquirer in acquirers:
                acquirers_per_code.setdefault(acquirer.company_id.id, {}).setdefault(
                    acquirer.provider, set()
                ).add(acquirer.id)
            ...
            for (
                ...
                if is_electronic:
                    journal_ids = (
                        ...
                        .setdefault(acquirer_id, set())
                    )
                else:
                    journal_ids = values["company_journals"].setdefault(
                        company_id, set()
                    )
        ...
        for journal in self:
            ...
            for pay_method in pay_methods:
                ...
                if values["mode"] == "unique":
                    ...
                    for acquirer_id in acquirers_per_code.get(company.id, {}).get(
                        pay_method.code, set()
                    ):
                        already_linked_journal_ids = values["company_journals"].get(
                            company.id, {}
                        ).get(acquirer_id, set()) - {journal._origin.id}
                        if (
                            not already_linked_journal_ids
                            and acquirer_id not in protected_acquirer_ids
                        ):
                            commands.append(Command.link(pay_method.id))
    ...
    def _check_payment_method_line_ids_multiplicity(self):
        ...
        self.env["account.payment.method.line"].flush(
            ["payment_method_id", "journal_id", "name"]
        )
