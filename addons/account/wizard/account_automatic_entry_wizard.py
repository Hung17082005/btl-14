        ...
        for date, grouped_lines in groupby(
            ...
            move_data[date] = {
                ...
                "ref": self._format_strings(
                    _(
                        "Adjusting Entry of {date} ({percent:.2f}% recognized on {new_date})"
                    ),
    ...
    def _format_transfer_source_log(self, balances_per_account, transfer_move):
        ...
        for account, balance in balances_per_account.items():
            if account != self.destination_account_id:
                content += self._format_strings(
                    transfer_format % account.display_name, transfer_move, balance
                )
