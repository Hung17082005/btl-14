...
class TaxReportTest(AccountTestInvoicingCommon):
    ...
    def test_write_single_line_tagname(self):
        ...
        self.assertEqual(
            len(self._get_tax_tags(tag_name=original_tag_name)),
            0,
    ...
    def test_write_single_line_remove_tagname(self):
        ...
        self.assertEqual(
            len(self._get_tax_tags(tag_name=tag_name_before)),
            0,
    ...
    def test_unlink_report_line_tags_used_by_amls(self):
                        ...
                        0,
                        {
                            ...
                            "tag_ids": [
                                (
                                    6,
                                    0,
                                    self.tax_report_line_1_55.tag_ids.filtered(
        ...
        self.assertEqual(
            ...
            [False],
            "Unlinking a report line should keep the tag if it was used on move lines, and unlink it otherwise.",
        )
        self.assertEqual(
            tags_after.mapped("active"),
            [False],
            "Unlinking a report line should archive the tag if it was used on move lines, and unlink it otherwise.",
        )
        self.assertEqual(
    ...
    def test_unlink_report_line_tags_used_by_other_report_line(self):
        ...
        self.assertEqual(
            len(tags_after),
            len(tags_before),
            "Unlinking a report line whose tags are used by another line should not delete them.",
        )
        self.assertEqual(
            len(tags_archived_after),
            len(tags_archived_before),
            "Unlinking a report line whose tags are used by another line should not archive them.",
        )
    ...
    def test_tag_recreation_archived(self):
        ...
        self.assertEqual(
            len(tags_after),
            2,
            "When creating a tax report line with an archived tag and its complement doesn't exist, it should be re-created.",
        )
        self.assertEqual(
