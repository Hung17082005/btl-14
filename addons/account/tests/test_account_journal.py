...
class TestAccountJournal(AccountTestInvoicingCommon):
    ...
    def test_default_account_type_control_create_journal_entry(self):
        ...
        self.company_data["default_journal_misc"].default_account_id = (
            self.company_data["default_account_expense"].id
        )

        # Should fail because 'default_account_revenue' type is not allowed.
        self.company_data["default_journal_misc"].type_control_ids |= self.company_data[
    ...
    def test_account_and_type_control_on_journal(self):
        ...
        self.env["account.move"].create(
            move_vals
        ).unlink()  # (unlink to allow modifying the allowed accounts below)
        ...
        self.company_data["default_journal_misc"].account_control_ids = (
            self.company_data["default_account_payable"].ids
        )
