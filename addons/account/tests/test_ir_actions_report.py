...
class TestIrActionsReport(AccountTestInvoicingCommon):
    ...
    def test_download_one_corrupted_pdf(self):
        ...
        test_record_report = (
            ...
            ._render_qweb_pdf(in_invoice_1.id)
        )
    ...
    def test_download_one_encrypted_pdf(self):
        ...
        with self.assertRaises(UserError):
            self.env.ref("account.action_account_original_vendor_bill").with_context(
                force_report_rendering=True
            )._render_qweb_pdf(res_ids=[in_invoice_1.id, in_invoice_2.id])
