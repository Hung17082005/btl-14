...
class TestChartTemplate(TransactionCase):
    ...
    @classmethod
    def _create_group_tax_template(
        cls, tax_template_xmlid, name, chart_template_id=None, active=True
    ):
        children_1 = cls._create_tax_template(
            f"{tax_template_xmlid}_children1", f"{name}_children_1", 10, active=active
        )
        children_2 = cls._create_tax_template(
            f"{tax_template_xmlid}_children2", f"{name}_children_2", 15, active=active
        )
            ...
            [
                {
                    ...
                    "values": {
                        ...
                        "children_tax_ids": [
                            Command.set((children_1 + children_2).ids)
                        ],
                    },
                }
            ]
                            ...
                            Command.create(
                                {
                                    "factor_percent": 100,
                                    "account_id": (
                                        account_template.id if account_data else None
                                    ),
    ...
    def test_update_taxes_new_template(self):
        ...
        tax_template_4 = self._create_tax_template(
            "account.test_tax_4_template",
            "Tax name 4",
            4,
            account_data={"name": "account_name_4", "code": "TACT"},
        )
        ...
        self.assertEqual(
            taxes.invoice_repartition_line_ids.account_id.name, "account_name_4"
        )
        self.assertRecordValues(
    ...
    def test_update_taxes_children_tax_ids(self):
        ...
        self._create_group_tax_template(
            "account.test_group_tax_test_template",
            group_tax_name,
        ...
        self.assertEqual(
            len(parent_tax), 1, "The parent tax should have been re-created."
        )
        self.assertEqual(
            parent_tax.children_tax_ids.ids,
            children_taxes.ids,
    ...
    def test_update_taxes_children_tax_ids_inactive(self):
        ...
        self._create_group_tax_template(
            "account.test_group_tax_test_template_inactive",
            group_tax_name,
            chart_template_id=self.chart_template.id,
            active=False,
        )
