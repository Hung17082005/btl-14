...
class TestAccountPaymentRegister(AccountTestInvoicingCommon):
    ...
    def test_register_payment_multi_currency_conversion(self):
        ...
        data = [
            (
                ...
                [1000.0, 666.67],
                "invoice is in foreign currency and payment is in company currency",
            ),
            (
                self.out_invoice_3,
                self.currency_data["currency"],
                [24.02, 36.03],
                "invoice is in company currency and payment is in foreign currency",
            ),
            (
                self.out_invoice_2,
                self.currency_data_3["currency"],
                [10.0, 2000.0],
                "invoice and payment are in different foreign currencies",
            ),
        ]
        ...
        for invoice, pay_currency, expected_amounts, msg in data:
            ...
            for d, expected in zip(date, expected_amounts):
                ...
                self.assertEqual(
                    wizard.amount,
                    expected,
                    f"Amount in payment wizard when {msg} is not correct",
                )
    ...
    def test_suggested_default_partner_bank_inbound_payment(self):
        ...
        self.bank_journal_2.bank_account_id = self.out_invoice_1.partner_bank_id = (
            ...
            [
                {
                    ...
                    "partner_id": self.partner_a.id,
                    "invoice_line_ids": [
                        (0, 0, {"product_id": self.product_a.id, "price_unit": 1600.0})
                    ],
                },
                {
                    "move_type": "in_refund",
