...
class AccountMove(models.Model):
    ...
    def _add_edi_tax_values(
        ...
        if serialized_grouping_key not in results["tax_details"]:
            ...
            if key_by_tax:
                ...
                add_to_base_amount = tax_values["base_line_id"] not in set(
    ...
    def _prepare_edi_tax_details(
        ...
        def compute_invoice_lines_tax_values_dict_from_compute_all(invoice_lines):
            ...
            for invoice_line in invoice_lines:
                ...
                for tax_res in taxes_res["taxes"]:
                    ...
                    if (
                        self.company_id.tax_calculation_rounding_method
                        == "round_per_line"
                    ):
        ...
        # (in the SQL query)
        if (
            compute_mode == "compute_all"
            and self.env.company.tax_calculation_rounding_method == "round_globally"
        ):
            ...
            for line, tax_values_list in invoice_lines_tax_values_dict.items():
                ...
                for tax_values in tax_values_list:
                    grouping_key = frozendict(
                        self._get_tax_grouping_key_from_base_line(
                            line, tax_values["tax_vals"]
                        )
                    )
                    total_amounts = amount_per_tax_repartition_line_id[grouping_key]
                    total_amounts["tax_amount_currency"] += tax_values[
                        "tax_amount_currency"
                    ]
            ...
            for key, values in amount_per_tax_repartition_line_id.items():
                currency = (
                    self.env["res.currency"].browse(key["currency_id"]) or comp_currency
                )
                values["tax_amount_rounded"] = comp_currency.round(values["tax_amount"])
                values["tax_amount_currency_rounded"] = currency.round(
            ...
            for key, values in amount_per_tax_repartition_line_id.items():
                ...
                for currency, amount_field in (
                    (comp_currency, "tax_amount"),
                    (foreign_currency, "tax_amount_currency"),
                ):
                    ...
                    nb_cents_per_tax_values = math.floor(
                        nb_error / len(tax_values_list)
                    )
                    ...
                    for tax_values in tax_values_list:
                        ...
                        abs_delta_to_add = min(
                            abs_diff, currency.rounding * nb_amount_curr_cent
                        )
...
class AccountMoveLine(models.Model):
    ...
    def _prepare_edi_vals_to_export(self):
        ...
        res = {
            ...
            "price_subtotal_unit": (
                self.currency_id.round(self.price_subtotal / self.quantity)
                if self.quantity
                else 0.0
            ),
            "price_total_unit": (
                self.currency_id.round(self.price_total / self.quantity)
                if self.quantity
                else 0.0
            ),
            ...
            "gross_price_total_unit": (
                self.currency_id.round(gross_price_subtotal / self.quantity)
                if self.quantity
                else 0.0
            ),
