...
class AccountPayment(models.Model):
    ...
    def _compute_edi_show_cancel_button(self):
        for payment in self:
            ...
            payment.edi_show_cancel_button = any(
                [
                    doc.edi_format_id._needs_web_services() and doc.state == "sent"
    ...
    def _compute_edi_show_abandon_cancel_button(self):
        for payment in self:
            payment.edi_show_abandon_cancel_button = any(
                doc.edi_format_id._needs_web_services() and doc.state == "to_cancel"
                for doc in payment.edi_document_ids
            )
