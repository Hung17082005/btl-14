...
class TestAccessRights(TransactionCase):
    ...
    def test_hide_sensitive_fields_private_events_from_uninvited_admins(self):
        ...
        readed_event = john_private_evt.with_user(self.admin_user).read(
            sensitive_fields + ["name"]
        )
        search_readed_event = (
            self.env["calendar.event"]
            .with_user(self.admin_user)
            .search_read([private_event_domain])
        )
        ...
        for field in sensitive_stored_fields:
            ...
            check_filtered_event = searched_event.with_user(self.admin_user).filtered(
                lambda ev: ev.id == john_private_evt.id
            )
            check_filtered_domain = searched_event.with_user(
                self.admin_user
            ).filtered_domain([private_event_domain])
            check_search_query = (
                self.env["calendar.event"]
                .with_user(self.admin_user)
                ._search([private_event_domain])
            )
            check_search_object = (
                self.env["calendar.event"]
                .with_user(self.admin_user)
                .browse(check_search_query)
            )
            check_read_group = (
                self.env["calendar.event"]
                .with_user(self.admin_user)
                .read_group([private_event_domain], [field], [field])
            )
            ...
            if field == "name":
                ...
                self.assertEqual(
                    check_mapped_event,
                    ["Busy"],
                    "Private event name should be shown as Busy using the mapped function.",
                )
                self.assertEqual(
                    check_filtered_event.name,
                    "Busy",
                    "Private event name should be shown as Busy using the filtered function.",
                )
                self.assertEqual(
                    check_filtered_domain.name,
                    "Busy",
                    "Private event name should be shown as Busy using the filtered_domain function.",
                )
                self.assertEqual(
                    check_search_object.name,
                    "Busy",
                ...
                self.assertFalse(
                    check_mapped_event[0],
                    'Private event field "%s" should be hidden when using the mapped function.'
                    % field,
                )
                self.assertFalse(
                    check_filtered_event[field],
                    'Private event field "%s" should be hidden when using the filtered function.'
                    % field,
                )
                self.assertFalse(
                    check_filtered_domain[field],
                    'Private event field "%s" should be hidden when using the filtered_domain function.'
                    % field,
                )
                self.assertFalse(
                    check_search_object[field],
                    'Private event field "%s" should be hidden when using the _search function.'
                    % field,
                )

            # Private events are excluded from read_group by default, ensure that we do not fetch it.
            self.assertFalse(
                len(check_read_group),
                "Private event should be hidden using the function _read_group.",
            )
