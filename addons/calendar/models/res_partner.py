...
class Partner(models.Model):
    ...
    def _compute_meeting(self):
        if self.ids:
            all_partners = self.with_context(active_test=False).search_read(
                [("id", "child_of", self.ids)], ["parent_id"]
            )
            all_partners_parents = {
                p["id"]: p["parent_id"][0] for p in all_partners if p.get("parent_id")
            }
            ...
            """.format(subquery),
                [tuple(p["id"] for p in all_partners)],
            )
            ...
            for meeting_pid in set(meetings):
                ...
                while partner_id in all_partners_parents:
                    ...
                    if partner_id in self.ids:
                        meetings[partner_id] = (
                            meetings.get(partner_id, set()) | meetings[meeting_pid]
                        )
                ...
                attendees_details.append(
                    {
                        ...
                        "is_organizer": (
                            ...
                            == attendee.event_id.user_id.partner_id
                            else 0
                        ),
                    }
                )
