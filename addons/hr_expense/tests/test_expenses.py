...
class TestExpenses(TestExpenseCommon):
    ...
    def test_account_entry_multi_currency(self):
        ...
        self.assertEqual(
            expense.amount_residual, 350.0, "Expense sheet amount due is not correct"
        )
        ...
        self.assertDictEqual(
            stub_line[0],
            {
                ...
                "amount_total": formatLang(
                    self.env, 11.0, currency_obj=self.env.company.currency_id
                ),
                "amount_residual": "-",
    ...
    def _test_reset_move_to_draft(self, payment_mode):
        ...
        if payment_mode == "own_account":
            self.assertEqual(
                expense.state,
                "approved",
                "Expense state must be draft after posting move",
            )
            self.assertEqual(
                expense_sheet.state,
                "post",
                "Sheet state must be draft after posting move",
            )
    ...
    def test_expense_from_attachments(self):
        ...
        product.property_account_expense_id = self.company_data[
        ...
        self.assertEqual(
            ...
            "The expense account should be the default one of the product",
        )
    ...
    def test_create_report_name(self):
        ...
        sheet_name = expense_with_date_1._get_default_expense_sheet_values()[
            "default_name"
        ]
        self.assertEqual(
        ...
        sheet_name = expense_without_date._get_default_expense_sheet_values()[
            "default_name"
        ]
        self.assertEqual(
        ...
        self.assertFalse(
            sheet_name,
            "The report name should be empty if one of the expenses doesn't have a date",
        )
        expenses.date = False
        sheet_name = expenses._get_default_expense_sheet_values()["default_name"]
        self.assertFalse(
