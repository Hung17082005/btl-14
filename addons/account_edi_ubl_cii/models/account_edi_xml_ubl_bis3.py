...
class AccountEdiXmlUBLBIS3(models.AbstractModel):
    ...
    * Official doc for EHF Billing 3.0 is the OpenPeppol BIS 3 doc +
    ...
    def _get_invoice_line_allowance_vals_list(self, line, tax_values_list=None):
        # EXTENDS account.edi.xml.ubl_21
        vals_list = super()._get_invoice_line_allowance_vals_list(
            line, tax_values_list=tax_values_list
        )
    ...
    def _get_invoice_line_vals(self, line, taxes_vals):
        ...
        if (
            line.currency_id.compare_amounts(vals["price_vals"]["price_amount"], 0)
            == -1
        ):
    ...
    def _invoice_constraints_cen_en16931_ubl(self, invoice, vals):
        ...
        constraints = {
            ...
            "cen_en16931_payment_account_identifier": (
                self._check_required_fields(invoice, "partner_bank_id")
        ...
        for line in invoice.invoice_line_ids.filtered(
            ...
            if (
                len(
                    line.tax_ids.flatten_taxes_hierarchy().filtered(
        ...
        for role in ("supplier", "customer"):
            constraints[f"cen_en16931_{role}_country"] = self._check_required_fields(
                vals[role], "country_id"
            )
            scheme_vals = vals["vals"][f"accounting_{role}_party_vals"]["party_vals"][
                "party_tax_scheme_vals"
            ][-1:]
            if (
                not (
                    scheme_vals
                    and scheme_vals[0]["company_id"]
                    and scheme_vals[0]["company_id"][:2].isalpha()
                )
            ...
            constraints.update(
                {
                    ...
                    "nl_r_001": (
                        self._check_required_fields(invoice, "ref")
