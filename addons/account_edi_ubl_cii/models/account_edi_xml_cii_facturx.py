...
class AccountEdiXmlCII(models.AbstractModel):
        ...
        constraints.update(
            {
                ...
                "intracom_seller_vat": (
                    self._check_required_fields(vals["record"]["company_id"], "vat")
                ...
                "igic_tax_rate": (
                    self._check_non_0_rate_tax(vals)
                    if vals["record"]["commercial_partner_id"]["country_id"]["code"]
                    == "ES"
                    and vals["record"]["commercial_partner_id"]["zip"]
                    and vals["record"]["commercial_partner_id"]["zip"][:2]
                    in ["35", "38"]
                    else None
                ),
            }
    ...
    def _check_non_0_rate_tax(self, vals):
        for line_vals in vals["invoice_line_vals_list"]:
            tax_rate_list = (
                line_vals["line"].tax_ids.flatten_taxes_hierarchy().mapped("amount")
            )
    ...
    def _get_exchanged_document_vals(self, invoice):
        return {
            ...
            "included_note": (
                html2plaintext(invoice.narration) if invoice.narration else ""
            ),
        }
    ...
    def _export_invoice_vals(self, invoice):
        ...
        template_values = {
            **invoice._prepare_edi_vals_to_export(),
            ...
            "contract_reference": (
                ...
                if "contract_reference" in invoice._fields
                and invoice.contract_reference
                else ""
            ),
        }
        ...
        for tax_detail_vals in template_values["tax_details"]["tax_details"].values():
            ...
            tax_detail_vals["calculated_amount"] = (
                template_values["balance_multiplicator"] * amount_currency
        ...
        if supplier.country_id.code == "DE":
            template_values["document_context_id"] = (
        ...
        for line_vals in template_values["invoice_line_vals_list"]:
            ...
            for grouping_key, tax_detail in tax_details["invoice_line_tax_details"][
                line_vals["line"]
            ]["tax_details"].items():
        ...
        template_values["tax_basis_total_amount"] = (
            balance_sign * tax_details["base_amount_currency"]
        )
        template_values["tax_total_amount"] = (
            balance_sign * tax_details["tax_amount_currency"]
        )
    ...
    def _import_fill_invoice_form(self, journal, tree, invoice_form, qty_factor):
        ...
        if bank_details:
            self._import_retrieve_and_fill_partner_bank_details(
                invoice_form, bank_details=bank_details
            )
        ...
        logs += self._import_fill_invoice_allowance_charge(
            tree, invoice_form, journal, qty_factor
        )
            ...
            for i, invl_el in enumerate(line_nodes):
                with invoice_form.invoice_line_ids.new() as invoice_line_form:
                    ...
                    invl_logs = self._import_fill_invoice_line_form(
                        journal, invl_el, invoice_form, invoice_line_form, qty_factor
                    )
                    logs += invl_logs
    ...
    def _import_fill_invoice_line_form(
        ...
        inv_line_vals = self._import_fill_invoice_line_values(
            tree, xpath_dict, invoice_line_form, qty_factor
        )
        # retrieve tax nodes
