    ...
    def _compute_remaining_hours(self):
        timesheet_read_group = self.env["account.analytic.line"].read_group(
            domain=[
                ...
                ("task_id", "!=", False),
                "|",
                ("task_id.stage_id.fold", "=", False),
                ("task_id.stage_id", "=", False),
            ],
            fields=["effective_hours:sum(unit_amount)"],
            groupby="project_id",
        )
        ...
        effective_hours_per_project_id = {
            res["project_id"][0]: res["effective_hours"] for res in timesheet_read_group
        }
        planned_hours_per_project_id = {
            res["project_id"][0]: res["planned_hours"] for res in task_read_group
        }
        for project in self:
            ...
            project.remaining_hours = (
                planned_hours - effective_hours if planned_hours else 0.0
            )
