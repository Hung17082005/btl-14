    ...
    def _compute_shipping_weight(self):
        for picking in self:
            ...
            picking.shipping_weight = picking.weight_bulk + sum(
                [
                    pack.shipping_weight or pack.weight
                    for pack in picking.package_ids.sudo()
                ]
            )
    ...
    def _pre_put_in_pack_hook(self, move_line_ids):
        ...
        if not res:
            if move_line_ids.carrier_id:
                if len(move_line_ids.carrier_id) > 1 or any(
                    not ml.carrier_id for ml in move_line_ids
                ):
                ...
                return self._set_delivery_package_type(
                    batch_pack=len(move_line_ids.picking_id) > 1
                )
        else:
            return res
    ...
    def _add_delivery_cost_to_so(self):
        ...
        if (
            ...
            if not delivery_lines:
                delivery_lines = [
                    sale_order._create_delivery_line(
