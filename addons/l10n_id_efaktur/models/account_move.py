...
class AccountMove(models.Model):
    ...
    def _compute_kode_transaksi(self):
        for move in self:
            move.l10n_id_kode_transaksi = (
                move.partner_id.commercial_partner_id.l10n_id_kode_transaksi
            )
    ...
    def _generate_efaktur_invoice(self, delimiter):
        ...
        for move in self.filtered(lambda m: m.state == "posted"):
            ...
            nik = (
                str(commercial_partner.l10n_id_nik)
            ...
            if not invoice_npwp:
                ...
                msg = _(
                    "Please make sure that you've input the appropriate NPWP or NIK for the following customer"
                )
            ...
            if (
                invoice_npwp[:15] == "000000000000000"
                and commercial_partner.l10n_id_nik
            ):
                etax_name = "%s#NIK#NAMA#%s" % (
                    commercial_partner.l10n_id_nik,
                    etax_name,
                )
            ...
            eTax["ALAMAT_LENGKAP"] = (
                ...
                else commercial_partner.l10n_id_tax_address or street
            )
            eTax["JUMLAH_DPP"] = int(
                float_round(move.amount_untaxed, 0)
            )  # currency rounded to the unit
            eTax["JUMLAH_PPN"] = int(
                float_round(move.amount_tax, 0, rounding_method="DOWN")
            )  # tax amount ALWAYS rounded down
            ...
            eTax["FG_UANG_MUKA"] = 0
            eTax["UANG_MUKA_DPP"] = float_repr(
                abs(sum(lines.mapped(lambda l: float_round(l.price_subtotal, 0)))), 0
            )
            ...
            for line in move.line_ids.filtered(
                ...
                line_dict = {
                    ...
                    "HARGA_SATUAN": float_repr(
                        float_round(invoice_line_unit_price, 0), 0
                    ),
                    "JUMLAH_BARANG": line.quantity,
                    "HARGA_TOTAL": float_repr(
                        float_round(invoice_line_total_price, 0), 0
                    ),
