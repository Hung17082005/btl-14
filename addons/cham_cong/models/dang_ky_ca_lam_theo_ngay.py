    ...
    def _check_ngay_lam(self):
        for record in self:
            if record.ngay_lam and record.dot_dang_ky_id:
                if (
                    record.ngay_lam < record.dot_dang_ky_id.ngay_bat_dau
                    or record.ngay_lam > record.dot_dang_ky_id.ngay_ket_thuc
                ):
                    raise ValidationError(
                        f"Ngày làm phải nằm trong khoảng thời gian của đợt đăng ký (từ {record.dot_dang_ky_id.ngay_bat_dau} đến {record.dot_dang_ky_id.ngay_ket_thuc})"
                    )
