...
class Employee(models.Model):
    ...
    def _get_expected_attendances(self, date_from, date_to, domain=None):
        self.ensure_one()
        valid_contracts = self.sudo()._get_contracts(
        ...
        for contract in valid_contracts:
            contract_start = datetime.combine(
                contract.date_start, time.min, employee_tz
            )
            contract_end = datetime.combine(
                contract.date_end or date.max, time.max, employee_tz
            )
            calendar = (
                contract.resource_calendar_id
                or contract.company_id.resource_calendar_id
            )
