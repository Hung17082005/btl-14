...
class IoTboxHomepage(web.Home):
    ...
    def list_handlers(self, **post):
        ...
        if request.httprequest.method == "POST":
            ...
            need_config_save |= check_and_update_odoo_config_log_to_server_option(
                post.get("log-to-server")
                == "on"  # we use .get() as if the HTML checkbox is unchecked, no value is given in the POST request
            )
            ...
            for post_request_key, log_level_or_parent in post.items():
                ...
                if post_request_key == "root":
                    need_config_save |= self._update_logger_level(
                        "", log_level_or_parent, AVAILABLE_LOG_LEVELS
                    )
                elif post_request_key == "odoo":
                    need_config_save |= self._update_logger_level(
                        "odoo", log_level_or_parent, AVAILABLE_LOG_LEVELS
                    )
                    need_config_save |= self._update_logger_level(
                        "werkzeug",
                        (
                            log_level_or_parent
                            if log_level_or_parent != "debug"
                            else "info"
                        ),
                    ...
                    need_config_save |= self._update_logger_level(
                        logger_name,
                        log_level_or_parent,
                        AVAILABLE_LOG_LEVELS_WITH_PARENT,
                        "interfaces",
                    )
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        ...
        return (
            odoo_addon_handler_path in logging.Logger.manager.loggerDict.keys()
