...
FILE_TYPE_DICT = {
    ...
    "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet": (
        ...
        (
            ...
            else "openpyxl or xlrd >= 1.0.0 < 2.0"
        ),
    ),
...
class Import(models.TransientModel):
    ...
    def _read_xlsx(self, options):
        ...
        book = load_workbook(
            io.BytesIO(self.file or b""), read_only=True, data_only=True
        )
    ...
    def _convert_import_data(self, fields, options):
        ...
        if len(rows_to_import[0]) != len(fields):
            raise ImportValidationError(
                _(
                    "Error while importing records: all rows should be of the same size, but the title row has %d entries while the first row has %d. You may need to change the separator character.",
                    len(fields),
                    len(rows_to_import[0]),
                )
            )
